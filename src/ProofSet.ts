...
export class ProofSet {
   ...
   *
   * @param [documentLoader] {DocumentLoader} a custom document loader,
  ...
  async verify(
    document: any,
    {
      ...
      suite: LinkedDataProofSuite | LinkedDataProofSuite[]
      purpose: ProofPurpose
      documentLoader: DocumentLoader
    }
    ...
    try {
      ...
      const { proofSet, document: doc } = await _getProofs({ document })
      document = doc
      ...
      if (!verified) {
        ...
        const result: VerificationResult = { verified, results }
        if (errors.length > 0) {
...
async function _getProofs({
  ...
  document: any
}): Promise<{ proofSet: Proof[]; document: any }> {
...
async function _verify({
  ...
  await Promise.all(
    [...proofToSuite.entries()].map(async ([proof, suite]) => {
      let result
      try {
  ...
  await Promise.all(
    [...purposeToProofs.entries()].map(async ([purpose, proofs]) => {
...
function _makeSerializable(error: any): void {
...
async function _matchProofSet({
  ...
  purposeToProofs: Map<ProofPurpose, Proof[]>
  proofToSuite: Map<Proof, LinkedDataProofSuite>
  purpose: ProofPurpose
  proofSet: Proof[]
  suites: LinkedDataProofSuite[]
  suiteMatchQueue: Map<LinkedDataProofSuite, Map<Proof, Promise<boolean>>>
  document: any
  documentLoader: DocumentLoader
}): Promise<void> {
