...
export abstract class JwsLinkedDataSignature extends LinkedDataSignature {
  ...
  async sign({
    ...
    */
    ...
    const encodedSignature: string = base64url.encode(signature)
  ...
  async verifySignature({
    ...
    try {
      ...
      header = JSON.parse(utf8decoder.decode(base64url.decode(encodedHeader)))
    } catch (e: any) {
      throw new Error(`Could not parse JWS header: "${e.message as string}"`)
    }
